...
let map_file_name = "ShowMap24SmallCrop.png";
...
let showInfo = false;  //show image and diagram
...
class Project {
  ...
  repaint() {
    ...
    if (this == selectedProject || this.selected) {
    ...
    ellipse(this.x * width, this.y * height * 2, ellipseSize, ellipseSize);
    fill(255);
    textSize(fontSize)
    let keyText = this.printed_map_key + "";
    let keyTextWidth = textWidth(keyText);
    text(keyText, this.x * width - keyTextWidth / 2, this.y * height * 2 + 6);
    ...
    if (showTitle || this.selected) {
      fill(0);
      noStroke();
      text(shortName, this.x * width + ellipseSize / 2 + 1, this.y * height * 2 + 5);
    }

    // noFill();
    // stroke(255);
    // let ellipseSize = 10;
    // //if (!selectedProject) this.selected = false;

    // let shortName = this.name;
    // let textColor = color(0, 127, 127);

    // if (this.selected || this == taggingTarget) {
    //   fill(0, 255, 0);
    //   textColor = 0;
    //   if (this.searchedFor) fill(0, 255, 0);

    // } else {
    //   shortName = this.name.substring(0, 10);
    //   fill(255 - this.earlyRank, 0, 0);
    //   if (this.searchedFor) fill(0, 255, 0);
    // }
    // if (showTitle) {
    //   ellipseSize = 10;
    ...
    // }
    // if (showTitle || this == taggingTarget || this.selected) {
    //   noStroke();
    //   textSize(fontSize);
    //   fill(textColor)
    ...
    // }
    // if (showRequirements || this == taggingTarget) {
...
//     keyHolder.style.top = height / 2 + "px";
//     keyHolder.style.left = "0px";
//     keyHolder.style.width = width + "px";
//     keyHolder.style.height = height / 2 + "px";
...
//   //keyGraphics.background(220);
//   let topOffset = 0; //height / 2
//   keyRects = [];

//   let hSpacing = width / 12;
//   let keyY = topOffset; //height / 2
//   let keyX = 20;
//   //keyGraphics.textSize(fontSize);
//   for (key in byMapKey) {
//     let projectList = [];
...
//       console.log("thisProject", this, e.target);
...
//     //keyGraphics.text(key + "     " + shortName, keyX, keyY);
//     //thisRect = { x: keyX, y: keyY, w: 100, h: fontSize, projects: projectList };
//     //console.log("key", key, thisRect);
//     keyY += 20;
//     if (key % 35 == 0) {
...
//     //keyRects.push(thisRect);
//   }

//   let num = 1;
//   keyY = topOffset; //height / 2
//   keyX += hSpacing + hSpacing / 2;
//   // hSpacing = 160;
...
//     //thisDiv.onclick = function () { console.log("clicked on ", this.name); }
//     thisDiv.style.position = "absolute";
//     thisDiv.style.top = keyY + "px";
//     thisDiv.style.left = keyX + "px";
//     thisDiv.style.zIndex = "1000";
//     //thisDiv.style.color = "black";
//     //thisDiv.style.color = "green";
...
//       for (let i = 0; i < projectList.length; i++) {
//         let projectID = projectList[i];
//         //.log("hovering over", byProjectID[projectID].name, projectID);
//         byProjectID[projectID].searchedFor = true;
//       }
//     }

//     //keyGraphics.text(projectKeys, keyX + nameWidth + 2, keyY);
//     //thisRect = { x: keyX, y: keyY, w: 100, h: fontSize, projects: projectList };
//     //thisRect = { x: keyX + 10, y: keyY, w: hSpacing - 10, h: fontSize, projects: projectList };
//     keyY += 20;
//     if (num % 35 == 0) {
//       keyY = topOffset;//height / 2;
//       keyX += hSpacing;
...
function gotDBSync(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (!thisProject.mainImage) {
      image_url = "noImage";
    } else {
      howManyNeedLoading++
      image_url = thisProject.mainImage[0].document;
    }
    if (!thisProject.diagram) {
      diagram_url = "noImage";
    } else {
    ...
    if (thisProject.x_norm == -100) {
      ...
      //JSON.parse(thisProject.requirements)
      newProject = new Project(thisProject.x_norm, thisProject.y_norm, thisProject.project_name, thisProject.project_id, thisProject.w_norm, thisProject.h_norm, thisProject.printed_map_key, {}, thisProject.timestamp, image_url, diagram_url);
    }
    projects.push(newProject);
    byTimeStamp[thisProject.timestamp] = newProject;
    byXPos[newProject.x + i * 0.00000001] = newProject;
  ...
  */
  ...
  for (let i = 0; i < data.length; i++) {
    let thisProject = data[i];
    // if (thisProject.approved != 1) continue;
    let theName = thisProject.users[0].user_name.trim();
  ...
  */
  ...
  //createSelectPersonAndProjectInterface()
  //drawKey();
...
function mousePressed() {
  ...
  console.log("mouse pressed", selectedProject, mouseDownPos);
...
function createSelectPersonAndProjectInterface() {
  ...
  selectPerson.style.position = "absolute";
  selectPerson.style.zIndex = "1000";
  selectPerson.style.top = "0%";// 5 + "px"; //"80%";// ;
  selectPerson.style.left = "2%"; //320 + "px";  //"80%"; //
  ...
  selectProject.style.position = "absolute";
  selectProject.style.zIndex = "1000";
  selectProject.style.top = "0%"; //5 + "px";
  selectProject.style.left = "46%"; // + "px";
...
function selectClassEvent(e) {
  ...
  console.log("projectlist", projectsList, e.target.name);
  for (let i = 0; i < projects.length; i++) {
...
function mySelectEvent(e) {
  if (taggingTarget) { // requirementsMode == "tagging" &&
    ...
    let value = e.target.value;
    let name = this.name;
    console.log("which pulldown", name, value);
    for (let i = 0; i < projects.length; i++) {
